...
echo.
echo [FileHive] Starting backend and frontend...
...
start "FileHive Backend" cmd /k "cd /d ""%ROOT_DIR%"" && set PYTHONPATH=%ROOT_DIR% && ""%PYTHON_EXE%"" -m uvicorn app.main:app --host %HOST% --port %BACKEND_PORT% --reload"

echo [FileHive] Starting frontend window...
start "FileHive Frontend" cmd /k "cd /d ""%FRONTEND_DIR%"" && set VITE_API_BASE_URL=%FRONTEND_API_BASE% && set VITE_PROXY_TARGET=%FRONTEND_PROXY_TARGET% && ""%VITE_CMD%"" --host %HOST% --port %FRONTEND_PORT%"
